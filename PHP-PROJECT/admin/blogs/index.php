	...
	<div class="card card-outline card-primary">
		<div class="card-header">
			<div class="card-tools">
				<a class="btn btn-primary new_blog" href="javascript:void(0)"><i class="fa fa-plus"></i>&nbsp;Add New</a>
			</div>
		</div>
		<div class="card-body">
			<div class="table-responsive">
				<table class="table tabe-hover table-bordered table-compact" id="list">
					...
					<tbody>
						<?php
						$i = 1;
						$qry = $conn->query("SELECT * FROM `blogs` order by `title` asc");
...
<script>
	...
	$(document).ready(function(){
		$('.new_blog').click(function(){
			location.href = _base_url_+"?page=blogs/manage";
		})
		$('.manage_blog').click(function(){
			location.href = _base_url_+"?page=blogs/manage&id="+$(this).attr('data-id')
		})
