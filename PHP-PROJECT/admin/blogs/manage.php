...
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		<div class="card-header">
			<h5 class="card-title"><?php echo isset($id) ? "Manage": "Create" ?> Blog</h5>
		</div>
		<div class="card-body">
			<form id="blog">
				...
				<!-- <div class="row">
					<div class="col-sm-6">
						<div class="form-group">
							<label for="" class="control-label">Button Label</label>
				             <input name="button_label" type="text" class="form-control" value="<?php echo isset($button_label) ? $button_label : '' ?>">
						</div>
					</div>
					<div class="col-sm-6">
						<div class="form-group">
							<label for="" class="control-label">Button URL</label>
				             <input name="button_link" type="text" class="form-control" value="<?php echo isset($button_link) ? $button_link : '' ?>">
						</div>
					</div>
				</div> -->
			</form>
				...
				success:function(resp){
					if(resp != undefined){
						if(resp.status == 'success'){
							location.href=_base_url_+"?page=blogs";
						}else{
							alert_toast("An error occured",'error')
							console.log(resp);
						}
