...
<?php 
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * from signature_dishes where id = '{$_GET['id']}' ");
...
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		<div class="card-header">
			<h5 class="card-title"><?php echo isset($id) ? "Manage" : "Create" ?> Signature Dishes</h5>
		</div>
		<div class="card-body">
			<form id="signature_dish">
		...
		<div class="card-footer">
			<button class="btn btn-primary btn-sm" form="signature_dish"><?php echo isset($_GET['id']) ? "Update": "Save" ?></button>
			<a class="btn btn-primary btn-sm" href="./?page=signature_dishes">Cancel</a>
		</div>
	</div>
</div>
...
<script>
	...
	$(document).ready(function(){
		$('.select')
		$('#signature_dish').submit(function(e){
			e.preventDefault();
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Content.php?f=signature_dish",
				...
				success:function(resp){
					if(resp != undefined){
						if(resp.status == 'success'){
							location.href=_base_url_+"?page=signature_dishes";
						}else{
							alert_toast("An error occured",'error')
							console.log(resp);
						}
