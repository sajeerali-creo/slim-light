...
<?php 
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * from success_stories where id = '{$_GET['id']}' ");
...
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		<div class="card-header">
			<h5 class="card-title"><?php echo isset($id) ? "Manage" : "Create" ?> Success Story</h5>
		</div>
		<div class="card-body">
			<form id="success_story">
		...
		<div class="card-footer">
			<button class="btn btn-primary btn-sm" form="success_story"><?php echo isset($_GET['id']) ? "Update": "Save" ?></button>
			<a class="btn btn-primary btn-sm" href="./?page=success_stories">Cancel</a>
		</div>
	</div>
</div>
...
<script>
	...
	$(document).ready(function(){
		$('.select')
		$('#success_story').submit(function(e){
			e.preventDefault();
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Content.php?f=success_story",
				...
				success:function(resp){
					if(resp != undefined){
						if(resp.status == 'success'){
							location.href=_base_url_+"?page=success_stories";
						}else{
							alert_toast("An error occured",'error')
							console.log(resp);
						}
