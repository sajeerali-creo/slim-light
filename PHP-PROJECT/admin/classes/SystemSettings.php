...
class SystemSettings extends DBConnection{
	...
	function update_settings_info(){
		...
		if(isset($_FILES['sign_up_img_banner']) && $_FILES['sign_up_img_banner']['tmp_name'] != ''){
			$fname = 'uploads/'.strtotime(date('y-m-d H:i')).'_'.$_FILES['sign_up_img_banner']['name'];
			$move = move_uploaded_file($_FILES['sign_up_img_banner']['tmp_name'],'../'. $fname);
			if(isset($_SESSION['system_info']['banner2'])){
				$qry = $this->conn->query("UPDATE system_info set meta_value = '{$fname}' where meta_field = 'banner2' ");
				if(is_file('../'.$_SESSION['system_info']['banner2'])) unlink('../'.$_SESSION['system_info']['banner2']);
			}else{
				$qry = $this->conn->query("INSERT into system_info set meta_value = '{$fname}',meta_field = 'banner2' ");
			}
		}
